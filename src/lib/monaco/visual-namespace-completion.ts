...
const VISUAL_ATTRIBUTES = {
  layout: [
    {
      name: 'xywh',
      description:
        'Position and size as "x y width height" (space-separated pixels)',
    },
  ...
  style: [
    {
      name: 'rgb',
      description: 'Background color as RGB hex value (e.g., "FFEEEE")',
    },
  ...
  diagram: [
    {
      name: 'waypoints',
      description: 'Waypoints for transition paths (x1,y1 x2,y2 ...)',
    },
    { name: 'label-offset', description: 'Label positioning offset (x,y)' },
    {
      name: 'curve-type',
      description: 'Curve type for edges (smooth, step, straight, bezier)',
    },
    {
      name: 'marker-type',
      description: 'Arrow marker type (arrow, diamond, circle, none)',
    },
  ...
  actions: [
    {
      name: 'action-namespaces',
      description: 'Custom action namespaces (comma-separated)',
    },
 ...
 */
const VISUAL_SUPPORTED_ELEMENTS = [
  'scxml',
  'state',
  'parallel',
  'final',
 ...
 */
function hasVisualNamespace(documentText: string): {
...
function analyzeVisualContext(
  ...
  const inAttributeName =
    /\s+$/.test(beforeCursor) || /\s+\w*:?\w*$/.test(beforeCursor);
  ...
  const existingAttributes = attributeMatches.map((match) =>
    match.replace(/\s*=$/, '')
  );
...
export function createVisualNamespaceCompletionProvider(
  ...
  return {
    ...
    provideCompletionItems: (model, position, context, token) => {
      ...
      if (
        analysisContext.inAttributeName &&
        !analysisContext.inAttributeValue
      ) {
        // Add namespace declaration if not present and we're in root element
        if (
          !visualInfo.hasNamespace &&
          analysisContext.currentElement === 'scxml'
        ) {
        ...
        if (visualInfo.hasNamespace) {
          const visualSuggestions = getVisualAttributeSuggestions(
            visualInfo.prefix,
            monaco
          );
          // Filter out already existing attributes
          const filteredSuggestions = visualSuggestions.filter((suggestion) => {
            const labelText =
      ...
      if (analysisContext.inAttributeValue && visualInfo.hasNamespace) {
        ...
        const attributeMatch = analysisContext.beforeCursor.match(
          /\b(\w+:\w+)\s*=\s*["'][^"']*$/
        );
        if (attributeMatch) {
          const attributeName = attributeMatch[1];
          if (attributeName.startsWith(`${visualInfo.prefix}:`)) {
            const valueSuggestions = getVisualAttributeValueSuggestions(
              attributeName,
              visualInfo.prefix,
              monaco
            );
  ...
  return {
    provideHover: (model, position, token) => {
      ...
      if (
        !attributeMatch ||
        !line
          .substring(word.startColumn - 1, word.endColumn)
          .includes(word.word)
      ) {
        return null;
      }
